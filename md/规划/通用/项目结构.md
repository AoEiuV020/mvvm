...
2. **模块聚合**：依赖所有功能模块，提供统一的模块注册入口
3. **重新导出**：导出 ui-core 和所有功能模块的公共 API

```
sdkui/
  router/
    app_router           # 统一路由配置
  module_overrides       # 模块聚合
  sdkui                  # 导出所有模块
```

### 模块聚合机制

sdkui 依赖所有嵌入式组件模块，内部聚合好 Provider overrides：

- 提供默认 overrides 列表，包含所有已安装模块的默认实现
- 允许自定义替换部分模块实现

**App 层只需使用 sdkui 提供的默认配置，无需手动注册各模块。**

页面模块（login/chat）不需要 override，直接由路由引用。
